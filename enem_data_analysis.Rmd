...
```{r}
escolas %>% filter(Administração == "Pública", Ano == 2012) %>% summarize(média = round(mean(NU_MEDIA_CH,na.rm = T),2)) %>% dplyr::select(média) %>% pull() - escolas %>% filter(Administração == "Pública", Ano == 2013) %>% summarize(média = round(mean(NU_MEDIA_CH,na.rm = T),2)) %>% dplyr::select(média) %>% pull()

escolas %>% filter(Administração == "Privada") %>% group_by(Ano) %>% summarize(qtl = quantile(NU_MEDIA_LP,0.75,na.rm = T)) %>% filter(qtl == max(qtl)) %>% dplyr::select(Ano) %>% pull()
```
